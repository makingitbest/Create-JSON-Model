...
+ (void)createModelWithJsonData:(NSDictionary *)jsonData rootModelName:(NSString *)rootModelName controller:(UIViewController *)controller {
    ...
    if ([jsonData isKindOfClass:[NSDictionary class]]) {
        ...
        if (data && controller) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NodeModelRootNavigationController *nodeNC    = [[NodeModelRootNavigationController alloc] initWithRootViewController:nodeVC
                                                                                                                       rootNodeModel:nodeModel];
                [controller presentViewController:nodeNC animated:YES completion:nil];
            });
        }
    }
}
